...
function obj:_showContextMenu(row)
  ...
  local menuTable = {
      { title = "Insert all snippets with tab", fn = hs.fnutils.partial(self.pasteAllWithDelimiter, self, row, "\t") },
      { title = "Insert all snippets with line break", fn = hs.fnutils.partial(self.pasteAllWithDelimiter, self, row, "\n") },
      { title = "-" },
      { title = "Remove entry",   fn = hs.fnutils.partial(self.manageClip, self, row, 0) },
      { title = "Move entry to first position",   fn = hs.fnutils.partial(self.manageClip, self, row, -100)  },
